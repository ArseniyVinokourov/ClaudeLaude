...
def transcribe(audio_path: str,
               worker_analyzers: list[str] | None = None,
               bias_fillers: bool = False) -> dict | None:
    ...
    ``bias_fillers`` (#126) tells the worker to keep filler words (um/uh/э-э) in
    the transcript instead of normalizing them away — set when the ``fluency``
    analyzer is on (``speech.fluency_active``). It makes the transcript more
    verbatim, so it is off unless that analyzer wants it.
    """
